"""Enhanced Typer wrapper with sniff-cli detection and optional Tully tracking.
...
The base ``pip install sniff-cli`` install includes the CLI stack.
...
AUTO_ACTIVATE_SPEC_NAME: Final = ".sniff-cli.toml"
...
def _get_typer():
    ...
    if not _TYPER_AVAILABLE:
        raise ImportError(
            "typer is required for sniff_cli.Typer. "
            "Install or repair the package with: pip install --upgrade sniff-cli"
        )
    return _typer
...
def __getattr__(name: str):  # noqa: N807
    if name in TYPER_EXPORTS:
        ...
        globals()[name] = val
        return val
    raise AttributeError(f"module 'sniff_cli.typer_app' has no attribute {name!r}")


class Typer:
    """Enhanced Typer with sniff-cli detection and Tully integration.
    ...
    def _auto_activation_hook(self, ctx: Any) -> None:
        """Auto-activate environment from .sniff-cli.toml before each command."""
        import os
        from sniff_cli.activation import EnvironmentActivator
        from sniff_cli.envspec import find_envspec
        from sniff_cli.cli.errors import NotFoundError, DependencyError
        from sniff_cli.cli.styles import print_error
