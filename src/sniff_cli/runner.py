...
def _find_sniff_toml(start: Path) -> Path | None:
    """Walk up from start looking for .sniff-cli.toml."""
    current = start.resolve()
    for parent in [current, *current.parents]:
        candidate = parent / ".sniff-cli.toml"
        if candidate.is_file():
            return candidate
    return None
...
def _venv_executable(venv_path: Path, name: str) -> Path:
    ...
    if name == "python":
        return sniff_os.venv_python(venv_path)
    if name == "pip":
        return sniff_os.venv_pip(venv_path)
    return sniff_os.venv_bin_dir(venv_path) / name
...
def _bootstrap_venv(pyproject_path: Path, venv_path: Path) -> None:
    ...
    from sniff_cli._compat import load_toml
...
def _activate_sniff_env(sniff_toml_path: Path, script_dir: Path) -> None:
    """Load .sniff-cli.toml env vars and paths into os.environ."""
    from sniff_cli._compat import load_toml

    data = load_toml(sniff_toml_path)
    if data is None:
        return

    project_root = sniff_toml_path.parent
...
def run_script(script_path: str, args: list[str]) -> None:
    ...
    if not script.is_file():
        print(f"sniff-cli: script not found: {script_path}", file=sys.stderr)
        sys.exit(1)
    ...
    if pyproject is None:
        print(
            f"sniff-cli: no pyproject.toml found for {script_path}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Optionally activate .sniff-cli.toml environment
    sniff_toml = _find_sniff_toml(script.parent)
    if sniff_toml is not None:
        _activate_sniff_env(sniff_toml, script.parent)
