...
class BinaryInstaller:
    ...
    def install_wrapper(
        ...
        Args:
            ...
            spec_file: Path to .sniff-cli.toml (mutually exclusive with spec)
            ...
            if spec_file is not None:
                ...
                if found is None:
                    raise NotFoundError(
                        ...
                        hint="Provide spec or spec_file, or create .sniff-cli.toml",
                    )
    ...
    def install_python_shim(
        ...
        *,
        ...
        """Install a wrapper that runs a Python script through ``python -m sniff_cli``.

        This is the highest-friction-free path for Python CLIs that live in a
        normal Python project: the installed command reuses sniff-cli's
        ...
        if _find_pyproject(script.parent) is None:
            raise NotFoundError(
                ...
                hint="Add a pyproject.toml near the script or use --python with a .sniff-cli.toml config",
            )

        command = [sys.executable, "-m", "sniff_cli", str(script)]
...
def _render_command_wrapper(command: Sequence[str], *, project_name: str) -> str:
    ...
    if sniff_os.name == "windows":
        lines = [
            "@echo off",
            "setlocal",
            f"REM Wrapper for {project_name}",
            "REM Generated by sniff-cli",
