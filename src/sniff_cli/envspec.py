"""Environment specification parser for .sniff-cli.toml files."""
...
from sniff_cli._compat import tomllib, walk_up
...
class EnvironmentSpec:
    """Environment specification from .sniff-cli.toml."""
    ...
    def from_file(cls, path: Path) -> EnvironmentSpec:
        ...
        if not path.exists():
            ...
            raise ConfigError(
                ...
                hint="Run 'sniff init' to create a .sniff-cli.toml file",
            )
    ...
    def _from_dict(cls, data: dict[str, Any]) -> EnvironmentSpec:
        ...
        if not project.get("name"):
            ...
            raise ValidationError(
                ...
                hint="Add [project]\\nname = \"myproject\" to .sniff-cli.toml",
            )
...
def find_envspec(start_dir: Optional[Path] = None) -> Optional[Path]:
    """Find .sniff-cli.toml by walking up the directory tree."""
    return walk_up(Path(start_dir or Path.cwd()), ".sniff-cli.toml")
