...
variables, PATH entries, and conda configuration from a .sniff-cli.toml spec.
...
from .sniff_os import get_sniff_os
...
class WrapperGenerator:
    ...
    def generate(
        ...
        *,
        ...
        sniff_os = get_sniff_os()
        return sniff_os.render_wrapper(
            target=target,
            env_vars=env_vars,
    ...
    def from_spec(
        ...
        *,
        ...
        """Generate a wrapper from a ``.sniff-cli.toml`` spec (or ``EnvironmentSpec``).

        This resolves the full environment (conda prefix, env vars, PATH
        entries) via :class:`~sniff_cli.activation.EnvironmentActivator` and
        then delegates to :meth:`generate`.

        Args:
            spec_file: Path to a ``.sniff-cli.toml`` file, or an already-parsed
                :class:`~sniff_cli.envspec.EnvironmentSpec`.
    ...
    def install(
        ...
        *,
        ...
        Returns:
            An :class:`~sniff_cli.install.InstallResult` with the written path,
        ...
        sniff_os = get_sniff_os()
        wrapper_name = sniff_os.wrapper_filename(name)

        if install_dir is None:
            install_dir = sniff_os.default_wrapper_install_dir()
        ...
        sniff_os.make_wrapper_executable(wrapper_path)
    ...
    def uninstall(
        ...
        *,
        ...
        Returns:
            :class:`~sniff_cli.install.InstallResult` describing what was removed.
        """
        sniff_os = get_sniff_os()
        wrapper_name = sniff_os.wrapper_filename(name)
    ...
    def install_from_spec(
        ...
        *,
        ...
        This is the highest-level entry point: pass a ``.sniff-cli.toml`` path,
        the target binary, and a wrapper name, and get back an installed,
        executable wrapper.

        Args:
            spec_file: Path to ``.sniff-cli.toml`` (or an ``EnvironmentSpec``).
        ...
        Returns:
            :class:`~sniff_cli.install.InstallResult`.
        """
...
def _generate_cmd_script(
    *,
    ...
    lines: list[str] = [
        ...
        f"REM Generated by sniff-cli on {timestamp}",
        f"REM Regenerate with: sniff wrap {project_name} {target}"
