...
class LibraryPathResolver:
    ...
    def configure_builder(self, builder: object) -> None:
        ...
        Works with both sniff_cli.env.EnvVarBuilder and sniff_cli.toolchain.EnvVarBuilder.

        Args:
            builder: An EnvVarBuilder instance (from sniff_cli.env or sniff_cli.toolchain).
        """
        name, value = self.to_env_var()
        if not value:
            return
        # sniff_cli.env.EnvVarBuilder uses set_from_path / set
        if hasattr(builder, "set_from_path"):
            info = self.resolve()
            builder.set_from_path(name, list(info.paths))
        # sniff_cli.toolchain.EnvVarBuilder uses prepend_var
