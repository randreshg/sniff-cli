"""Multi-format output handler for sniff-cli CLI applications.
...
Usage::
    ...
    from sniff_cli.cli.output import OutputFormatter, OutputFormat, print_dep_results

    fmt = OutputFormatter(format=OutputFormat.TABLE, verbose=True)
    fmt.print_result({"name": "sniff-cli", "version": "3.0.0"}, title="Package Info")
...
class OutputFormatter:
    ...
    mode, styled helpers from :mod:`sniff_cli.cli.styles` are used for status
...
def print_dep_results(
    ...
    *,
    ...
    Iterates over a list of :class:`~sniff_cli.deps.DependencyResult` objects,
    ...
    Args:
        results: Iterable of ``DependencyResult`` objects (from
            :class:`~sniff_cli.deps.DependencyChecker`).
    ...
    Returns:
        ...
        dependencies, ready to pass to :func:`~sniff_cli.cli.styles.print_numbered_list`.

    Example::

        from sniff_cli import DependencyChecker, print_dep_results
