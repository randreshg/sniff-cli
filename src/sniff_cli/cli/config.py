...
from sniff_cli._compat import tomllib, load_toml, deep_merge, walk_up

# TOML writing: installed with the base sniff-cli package
try:
    import tomli_w
except ImportError:
...
def _load_toml(path: Path) -> dict[str, Any]:
    ...
    :func:`sniff_cli._compat.load_toml`.
    """
    if tomllib is None:
        return {}
...
def _deep_merge(target: dict[str, Any], source: dict[str, Any]) -> None:
    ...
    Thin wrapper kept for backward compatibility; delegates to
    :func:`sniff_cli._compat.deep_merge`.
    """
