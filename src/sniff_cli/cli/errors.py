"""Structured error handling for sniff-cli CLI applications.
...
Usage::
    ...
    from sniff_cli.cli.errors import NotFoundError, ValidationError
...
class SniffError(Exception):
    """Base exception for sniff-cli CLI errors.

    All sniff-cli exceptions carry:
...
class NotFoundError(SniffError):
...
class ValidationError(SniffError):
...
class DependencyError(SniffError):
...
class PermissionError(SniffError):
