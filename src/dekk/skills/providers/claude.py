...
GUARD_SCRIPT_NAME = "guard-build-tools.sh"
...
class ClaudeCodeAgent(DekkAgent):
        ...
        if enrichment.blocked_commands:
            ...
            guard_path.write_text(
                self._generate_guard_script(
                    enrichment.blocked_commands, enrichment.cli_name,
                ),
                encoding="utf-8",
            )
    ...
    @staticmethod
    def _generate_guard_script(blocked: list[str], cli_name: str | None) -> str:
        """Generate a bash script that blocks raw build tool invocations.

        Reads Claude Code PreToolUse hook stdin JSON, extracts the bash
        command, and exits 2 (blocking) when a blocked tool is detected.
        """
        alt = cli_name or "dekk"
        ...
        return textwrap.dedent(f"""\
            ...
            if echo "$CMD" | grep -qwE '{pattern}'; then
              echo "Use {alt} instead of raw build tools ({', '.join(blocked)})" >&2
              exit 2
            fi
    ...
    def _build_hooks_record(
        ...
        """Build Claude Code hooks record keyed by event name."""
    ...
    def clean(self, context: AgentContext) -> list[str]:
        ...
        if context.source_dir.is_dir():
            ...
            for hooks_file in (CLAUDE_HOOKS_JSON, GUARD_SCRIPT_NAME):
                removed.extend(
                    remove_file(
                        context.source_dir / CLAUDE_HOOKS_DIR / hooks_file,
                        f"{context.source_dir_name}/{CLAUDE_HOOKS_DIR}/{hooks_file}",
                    )
                )
