...
class LibraryPathResolver:
    ...
    def configure_builder(self, builder: object) -> None:
        ...
        Works with both sniff.env.EnvVarBuilder and sniff.toolchain.EnvVarBuilder.

        Args:
            builder: An EnvVarBuilder instance (from sniff.env or sniff.toolchain).
        """
        name, value = self.to_env_var()
        if not value:
            return
        # sniff.env.EnvVarBuilder uses set_from_path / set
        if hasattr(builder, "set_from_path"):
            info = self.resolve()
            builder.set_from_path(name, list(info.paths))
        # sniff.toolchain.EnvVarBuilder uses prepend_var
