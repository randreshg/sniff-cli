...
def _find_sniff_toml(start: Path) -> Path | None:
    """Walk up from start looking for .sniff.toml."""
    current = start.resolve()
    for parent in [current, *current.parents]:
        candidate = parent / ".sniff.toml"
        if candidate.is_file():
            return candidate
    return None
...
def _bootstrap_venv(pyproject_path: Path, venv_path: Path) -> None:
    ...
    if poetry:
        ...
        if result.returncode == 0 and (venv_path / "bin" / "python").is_file():
    ...
    from sniff._compat import load_toml
...
def _activate_sniff_env(sniff_toml_path: Path, script_dir: Path) -> None:
    """Load .sniff.toml env vars and paths into os.environ."""
    from sniff._compat import load_toml
...
def run_script(script_path: str, args: list[str]) -> None:
    ...
    if not script.is_file():
        print(f"sniff: script not found: {script_path}", file=sys.stderr)
        sys.exit(1)
    ...
    if pyproject is None:
        print(
            f"sniff: no pyproject.toml found for {script_path}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Optionally activate .sniff.toml environment
