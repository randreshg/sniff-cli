...
from sniff_cli.deps import DependencyResult, DependencySpec
from sniff_cli.diagnostic import (
    CheckRegistry,
    CheckResult,
    CheckStatus,
    DiagnosticReport,
...
from sniff_cli.diagnostic_checks import CIEnvironmentCheck, DependencyCheck, PlatformCheck
from sniff_cli.remediate import (
    DetectedIssue,
    FixResult,
    FixStatus,
    IssueSeverity,
...
def test_platform_check_reports_detected_platform_details():
    ...
    with patch("sniff_cli.diagnostic_checks.PlatformDetector.detect", return_value=fake_platform):
...
def test_platform_check_reports_detector_failures():
    with patch("sniff_cli.diagnostic_checks.PlatformDetector.detect", side_effect=RuntimeError("boom")):
        result = PlatformCheck().run()
...
def test_dependency_check_passes_when_dependency_is_found():
    ...
    with patch("sniff_cli.diagnostic_checks.DependencyChecker.check", return_value=dependency):
...
def test_dependency_check_warns_for_old_or_optional_dependencies():
    ...
    with patch("sniff_cli.diagnostic_checks.DependencyChecker.check", return_value=old_dependency):
        old_result = DependencyCheck(old_spec).run()
    with patch("sniff_cli.diagnostic_checks.DependencyChecker.check", return_value=missing_optional):
...
def test_dependency_check_fails_for_missing_required_or_checker_errors():
    ...
    with patch("sniff_cli.diagnostic_checks.DependencyChecker.check", return_value=missing_required):
        missing_result = DependencyCheck(DependencySpec("Git", "git")).run()
    with patch("sniff_cli.diagnostic_checks.DependencyChecker.check", side_effect=RuntimeError("boom")):
...
def test_ci_environment_check_skips_outside_ci():
    ...
    with patch("sniff_cli.diagnostic_checks.CIDetector.detect", return_value=fake_ci):
...
def test_ci_environment_check_reports_detector_failures():
    with patch("sniff_cli.diagnostic_checks.CIDetector.detect", side_effect=RuntimeError("boom")):
        result = CIEnvironmentCheck().run()
