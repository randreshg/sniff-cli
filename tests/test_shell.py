...
# ---------------------------------------------------------------------------
# Integration: APXM activation scenario
# ---------------------------------------------------------------------------


class TestApxmIntegration:
    """End-to-end test showing how APXM could use the shell module."""

    def test_detect_and_activate(self):
        """Simulate: detect shell, build activation script for APXM."""
        detector = ShellDetector()
        builder = ActivationScriptBuilder()

        # Force bash for deterministic test
        shell_info = detector.detect(shell_override="bash")
        assert shell_info.kind == ShellKind.BASH
        ...
        script = builder.build(config, shell_info.kind)

        # Verify the generated script matches what APXM currently does manually
        assert f'export MLIR_DIR="{prefix}/lib/cmake/mlir"' in script
        assert f'export LLVM_DIR="{prefix}/lib/cmake/llvm"' in script
        assert f'export PATH="{prefix}/bin:$PATH"' in script

    def test_tcsh_activation_for_current_env(self):
        """Test tcsh since that's the actual shell in the APXM dev environment."""
        builder = ActivationScriptBuilder()
        ...
        script = builder.build(config, ShellKind.TCSH)
        assert f'setenv MLIR_DIR "{prefix}/lib/cmake/mlir"' in script
        assert f'setenv PATH "{prefix}/bin:$PATH"' in script

    def test_full_completion_spec(self):
        """Generate completions for the actual APXM CLI commands."""
        gen = CompletionGenerator()
        spec = CompletionSpec(
            command="apxm",
            subcommands=(
                Subcommand(name="compile", description="Compile graph to artifact"),
                Subcommand(name="execute", description="Compile and run graph"),
                Subcommand(name="run", description="Run pre-compiled artifact"),
                Subcommand(name="doctor", description="Diagnose dependencies"),
                Subcommand(name="activate", description="Print shell exports"),
                Subcommand(name="install", description="Install conda env"),
                Subcommand(name="register", description="Manage credentials"),
            ),
        )

        for shell in (ShellKind.BASH, ShellKind.ZSH, ShellKind.FISH, ShellKind.POWERSHELL):
            script = gen.generate(spec, shell)
            assert "compile" in script
            assert "apxm" in script
